use macroquad::{
    color::Color, input::{KeyCode, MouseButton, is_key_down, is_key_pressed, is_mouse_button_pressed, mouse_position}, shapes::draw_rectangle, text::{draw_text, measure_text}, window::{screen_height, screen_width}
};
...
impl Menu {
        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            ...
            if x > MENU_OFFSET && x < screen_width() - MENU_OFFSET && y > START_ROOMS_Y {
                ...
                if idx < self.rooms.len() &&
                    let Err(e) = network.join_room(&self.rooms[idx].room_id, "eeemy".to_string()) {
                        eprintln!("Could not send join room request ({})", e);
                }
            }
        }
    }
    ...
    pub fn render(&self) {
        ...
        draw_text("Trap the mouse", x, MENU_OFFSET + 64.0, 64.0, Color::from_hex(0xEBF4DD));

        let mut room_y = START_ROOMS_Y;
        for (idx, room) in self.rooms.iter().enumerate() {
            draw_rectangle(
                MENU_OFFSET + 16.0, room_y, screen_width() - 2.0 * MENU_OFFSET - 32.0, ROOM_HEIGHT,
                Color::from_hex(0x5A7863)
            );

            draw_text(
                format!("\t\t\tRoom {}\t({}/2) players", idx + 1, room.player_count).as_str(),
                MENU_OFFSET, room_y + ROOM_HEIGHT * 0.5, 32.0, Color::from_hex(0xEBF4DD)
            );
