...
fn parse_line(line: &str) -> Option<Student> {
    ...
    for (k, value) in line.split(',').enumerate() {
...
fn p1(filepath: &str) -> Result<(), io::Error> {
    ...
    for line in s.lines() {
        if let Some(student) = parse_line(line) {
            if youngest.is_none() ||  student.age < youngest.as_ref().unwrap().age {
                youngest = Some(student.clone());
            }
            if oldest.is_none() ||  student.age > oldest.as_ref().unwrap().age {
...
fn set_pixels(canvas: &mut Canvas, pixels: &[(usize, usize, u8)]) {
    for pixel in pixels {
        canvas.pixels[pixel.0][pixel.1] = pixel.2 as char;
    }
}
...
fn p3(filepath: &str) -> Result<(), io::Error> {
